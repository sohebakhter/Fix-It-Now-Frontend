...
export default function AboutPage() {
  return (
    <main className="pb-20 pt-8 md:pt-12">
      <section className="relative overflow-hidden rounded-[2rem] border border-border bg-gradient-to-br from-sky-50 via-white to-indigo-50 p-6 shadow-sm md:p-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-slate-600 md:text-lg">
              At FixItNow, we make everyday home and business upkeep simple. From fast repairs to premium maintenance, we connect people with dependable professionals who show up ready to solve the problem.
            </p>
              ...
              <CardContent className="p-0">
                <div className="rounded-[1.5rem] bg-slate-900 p-5 text-white">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-sky-200">Service promise</p>
                      <h2 className="mt-2 text-2xl font-semibold">We make it right.</h2>
                    </div>
      ...
      <section className="mt-16">
        <div className="mb-7 text-center">
          <p className="text-sm font-medium uppercase tracking-[0.18em] text-sky-600">Why people choose us</p>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {strengths.map(({ icon: Icon, title, text }) => (
            <Card key={title} className="h-full border-border bg-white shadow-sm transition-transform hover:-translate-y-1">
      ...
      <section className="mt-16 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div className="rounded-[2rem] border border-border bg-slate-900 p-8 text-white shadow-xl">
          <p className="text-sm font-medium uppercase tracking-[0.18em] text-sky-300">Our process</p>
          <h2 className="mt-3 text-3xl font-bold">Simple, clear, and stress-free.</h2>
              ...
              <div key={step} className="flex gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="pt-1">
                  ...
                  <p className="mt-1 text-sm text-slate-300">
                    {index === 0 && "Share your request and let us understand the job scope."}
                    {index === 1 && "Receive a transparent estimate and recommended next steps."}
                    {index === 2 && "Enjoy expert execution and ongoing support when you need it."}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-6">
          <Card className="border-border bg-white shadow-sm">
            <CardContent className="p-6 md:p-8">
              <p className="text-sm font-medium uppercase tracking-[0.18em] text-violet-600">Our mission</p>
              <h3 className="mt-3 text-2xl font-bold text-slate-900">Better service, better peace of mind.</h3>
              <p className="mt-4 text-base leading-7 text-slate-600">
                We help homeowners and businesses stay protected, productive, and comfortable by making essential services easier to access, easier to understand, and easier to trust.
              </p>
            </CardContent>
          </Card>

          <Card className="border-border bg-gradient-to-r from-sky-50 to-indigo-50 shadow-sm">
            <CardContent className="flex flex-col gap-4 p-6 md:flex-row md:items-center md:justify-between md:p-8">
              <div>
                <p className="text-sm font-medium uppercase tracking-[0.18em] text-sky-700">Need a fresh start?</p>
                <h3 className="mt-2 text-2xl font-bold text-slate-900">Let’s find the right fix.</h3>
              </div>
      ...
      <section className="mt-16 rounded-[2rem] border border-border bg-white p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.18em] text-sky-600">Customer experience</p>
            <h2 className="mt-2 text-3xl font-bold text-slate-900">Real results with a human touch.</h2>
          </div>
          <div className="flex items-center gap-1 rounded-full bg-amber-50 px-3 py-2 text-amber-700">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className="h-4 w-4 fill-current" />
            ))}
            <span className="ml-1 text-sm font-medium">Rated by local customers</span>
          </div>
        </div>
      </section>
    </main>
